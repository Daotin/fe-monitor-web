...
## 第五阶段：SourceMap 管理页面开发（已完成）

### 完成内容

1. **Mock 数据扩展**

   - 创建了 SourceMap 管理页面的 mock 数据
   - 实现了 SourceMap 文件列表、上传和删除的模拟数据
   - 模拟了文件大小、版本号和状态等信息

2. **核心组件开发**

   - 实现了 SourceMapUploader 组件，支持文件选择和上传
   - 实现了 SourceMapList 组件，展示已上传的文件列表
   - 实现了文件大小格式化、状态标签等功能

3. **SourceMap 管理页面实现**

   - 集成了上述组件，构建完整的 SourceMap 管理页面
   - 实现了数据加载和错误处理
   - 实现了文件上传前的验证（文件类型、大小、版本号）
   - 实现了文件删除前的确认提示
...
- 使用 Element Plus 的 Table 组件实现文件列表
