...
const uploadMap = async () => {
  ...
  try {
    ...
    await uploadSourceMap(errorDetail.value.appId, sourceMapFile.value.raw as File)
              ...
              <div class="source-code-preview">
                <div class="code-line" :class="{ 'error-line': i === 5 }" v-for="(_, i) in 10" :key="i">
                  ...
                  <code class="code-content">
                    {{ i === 5 ? '// 这里是错误发生的行（实际内容需要从服务端获取）' : '// 源码行 ' + (frame.line - 5 + i) }}
                  </code>
                </div>
              </div>
            </div>
